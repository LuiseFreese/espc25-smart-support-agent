...
export async function runTicket(message, category) {
    ...
    if (storageAccountName && storageAccountKey) {
        ...
        try {
            ...
            const tableClient = new TableClient(`https://${storageAccountName}.table.core.windows.net`, 'SupportTickets', credential);
 ...
 */
export async function updateTicketWithAIResponse(ticketId, aiResponse, confidence) {
    const storageAccountName = process.env.STORAGE_ACCOUNT_NAME;
    const storageAccountKey = process.env.STORAGE_ACCOUNT_KEY;
    if (!storageAccountName || !storageAccountKey) {
